 ...
 * 对子
 * */
public class DuiZiCardEx_GD implements OpCardEx {
	...
	public boolean checkCardAndCompare(PK_OpCard opCard, PKRoomSetSound setSound) {
		...
		int leftCard = setSound.getSet().getSmallCard(tempOpCard.cardList);
		int rightCard = setSound.getSet().getSmallCard(tempCompareOpCard.cardList);
	...
	public ArrayList<PK_OpCard> getOpCardList(ArrayList<Integer> cardList, PKRoomSetSound setSound) {
		...
		if(haveRazzCardList.size() >= COUNT){
			outCardList.add(new PK_OpCard(haveRazzCardList.subList(0, COUNT)));
		}
		return outCardList;
	}
